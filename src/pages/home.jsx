    ...
    <main className="home">
      ...
      <section className="py-12 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 py-2">
          <h2 className="text-2xl text-primary-light mb-8 text-center flex justify-center items-center gap-3"><span className="border h-px w-20"></span> TDF in the Media <span className="border h-px w-20"></span></h2>
          <div className="flex justify-around items-center overflow-x-auto gap-2 md:gap-6 scrollbar-hidden p-1">
            {[
              { src: "/images/media/media1.webp", alt: "Barabanki News Coverage" },
              { src: "/images/media/media2.webp", alt: "Foundation Development News" },
              { src: "/images/media/media3.webp", alt: "Community Outreach Highlights" },
              { src: "/images/media/media4.webp", alt: "Menstrual Hygiene Awareness Workshop" },
            ].map((item, idx) => (
              <div key={idx} className="group shrink-0 relative overflow-hidden hover:rounded-xl shadow-md transition-all hover:shadow-xl">
                <img
                  loading="lazy"
                  src={item.src}
                  alt={item.alt}
      ...
      <section className="home-team">
        ...
        <div className=" bg-[linear-gradient(to_bottom,var(--primary)_50%,#ece4ef_50%)] h-fit py-10 px-5 md:px-10 mt-5">
          <div className="grid grid-cols-4 items-baseline justify-items-center gap-5 md:gap-20 bg-[#ece5f0] p-5 rounded-lg">
            {members.map((member, i) => (
              <figure key={i} className="[&>img]:rounded-full [&>img]:aspect-square [&>img]:object-cover [&>img]:max-h-36 [&>img]:object-top [&>img]:border-2 [&>img]:border-primary">
                <img loading="lazy" src={`${member.img}`} alt="" />
                <figcaption className="text-center mt-2 text-sm">{member.name}</figcaption>
                <figcaption className="text-center text-xs">{member.desig}</figcaption>
              </figure>
            ))}
          </div>
        </div>
